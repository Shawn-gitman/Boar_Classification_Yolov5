...
def plot_results_overlay(start=0, stop=0):  # from utils.general import *; plot_results_overlay()
    ...
    s = ['train', 'train', 'train_loss', 'Precision', 'mAP@0.5', 'val', 'val', 'precision', 'Recall', 'mAP@0.5:0.95']  # legends
    t = ['Box', 'Objectness', 'Loss-mAP@0.5:0.95', 'P-R', 'mAP-F1']  # titles
        ...
        for i in range(5):
            ...
            for j in [i, i + 5]:
                ...
                if j == 2:
                    ...
                    ax[i].set_ylabel('Train_Loss', color='b')
                    ...
                    ax[i].set_title(t[i])
                    ax[i].legend()
                    ax[i].set_ylabel('Y2 data', color='g') if i == 0 else None  # add filename
                    ax[i].set_ylabel('mAP@0.5:0.95', color='g')
                else:
                    y = results[j, x]
...
def plot_results_overlay_PR(start=0, stop=0):  # from utils.general import *; plot_results_overlay()
    # Plot training 'results*.txt', overlaying train and val losses
    s = ['train', 'train', 'train_loss', 'Precision', 'mAP@0.5', 'val', 'val', 'precision', 'Recall', 'mAP@0.5:0.95']  # legends
    t = ['Box', 'Objectness', 'Loss-mAP@0.5:0.95', 'P-R', 'mAP-F1']  # titles
    for f in sorted(glob.glob('results*.txt') + glob.glob('../../Downloads/results*.txt')):
